...
def topbar(title: str) -> str:
    return f'''<header class="topbar">
      ...
      <div class="top-actions"><button class="icon-btn" aria-label="Messages">◫</button><button class="icon-btn notify" aria-label="Notifications">♢<i>3</i></button><button class="btn primary">＋ New Incident</button>{avatar('Atchayam G','sm')}</div>
    </header>'''
...
def sidebar(active_slug: str) -> str:
    ...
    return f'''<aside class="sidebar">
      ...
      <div class="profile">{avatar('Atchayam G','md')}<div><strong>Atchayam G</strong><span>Operations Lead</span></div><span>⌄</span></div>
    </aside>'''
