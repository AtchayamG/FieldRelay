...
export function incidentApiErrorCode(error: unknown): string | undefined {
...
export function incidentApiErrorMessage(error: unknown, fallback: string): string {
  const candidate = error as HttpLikeError;
  const payload = candidate?.error as ErrorEnvelope | undefined;
  return (
...
export function incidentApiStatus(error: unknown): number | undefined {
